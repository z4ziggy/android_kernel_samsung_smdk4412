...
static struct wm8958_micd_rate c2_eur_det_rates[] = {
...
static struct wm8958_micd_rate c2_eur_jackdet_rates[] = {
	{ MIDAS_DEFAULT_MCLK2,     true,  0,  0 },
	{ MIDAS_DEFAULT_MCLK2,    false,  0,  0 },
...
static void c2_eur_gpio_init(void)
{
	int err;
...
static void c2_eur_micd_set_rate(struct snd_soc_codec *codec)
{
	...
	if (wm8994->jackdet) {
		rates = c2_eur_jackdet_rates;
		num_rates = ARRAY_SIZE(c2_eur_jackdet_rates);
		wm8994->pdata->micd_rates = c2_eur_jackdet_rates;
		wm8994->pdata->num_micd_rates = num_rates;
	} else {
		rates = c2_eur_det_rates;
		num_rates = ARRAY_SIZE(c2_eur_det_rates);
		wm8994->pdata->micd_rates = c2_eur_det_rates;
		wm8994->pdata->num_micd_rates = num_rates;
	}
...
static void c2_eur_micdet(u16 status, void *data)
{
	...
	if (!(status & WM8958_MICD_STS)) {
		if (!wm8994->jackdet) {
			...
			c2_eur_micd_set_rate(wm1811->codec);
...
static int c2_eur_wm1811_aif1_hw_params(struct snd_pcm_substream *substream,
...
/*
 * c2_eur WM1811 DAI operations.
 */
static struct snd_soc_ops c2_eur_wm1811_aif1_ops = {
	.hw_params = c2_eur_wm1811_aif1_hw_params,
};

static int c2_eur_wm1811_aif2_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static struct snd_soc_ops c2_eur_wm1811_aif2_ops = {
	.hw_params = c2_eur_wm1811_aif2_hw_params,
};

static int c2_eur_wm1811_aif3_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	pr_err("%s: enter\n", __func__);
	return 0;
...
static struct snd_soc_ops c2_eur_wm1811_aif3_ops = {
	.hw_params = c2_eur_wm1811_aif3_hw_params,
};

static const struct snd_kcontrol_new c2_eur_controls[] = {
...
const struct snd_soc_dapm_widget c2_eur_dapm_widgets[] = {
...
const struct snd_soc_dapm_route c2_eur_dapm_routes[] = {
	{ "HP", NULL, "HPOUT1L" },
	{ "HP", NULL, "HPOUT1R" },
...
static struct snd_soc_dai_driver c2_eur_ext_dai[] = {
	{
		.name = "c2_eur.cp",
		.playback = {
			.channels_min = 1,
...
static int c2_eur_wm1811_init_paiftx(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = snd_soc_add_controls(codec, c2_eur_controls,
					ARRAY_SIZE(c2_eur_controls));

	ret = snd_soc_dapm_new_controls(&codec->dapm, c2_eur_dapm_widgets,
					   ARRAY_SIZE(c2_eur_dapm_widgets));
	if (ret != 0)
		dev_err(codec->dev, "Failed to add DAPM widgets: %d\n", ret);

	ret = snd_soc_dapm_add_routes(&codec->dapm, c2_eur_dapm_routes,
					   ARRAY_SIZE(c2_eur_dapm_routes));
	if (ret != 0)
	...
	ret = snd_soc_jack_new(codec, "c2_eur Jack",
	...
	if (wm8994->revision > 1) {
		dev_info(codec->dev, "wm1811: Rev %c support mic detection\n",
			'A' + wm8994->revision);
		ret = wm8958_mic_detect(codec, &wm1811->jack, c2_eur_micdet,
			wm1811);
	...
	wake_lock_init(&wm1811->jackdet_wake_lock,
					WAKE_LOCK_SUSPEND, "c2_eur_jackdet");
...
static struct snd_soc_dai_link c2_eur_dai[] = {
	{ /* Sec_Fifo DAI i/f */
		.name = "Sec_FIFO TX",
		.stream_name = "Sec_Dai",
		...
		.init = c2_eur_wm1811_init_paiftx,
		.ops = &c2_eur_wm1811_aif1_ops,
	},
	{
		.name = "c2_eur_WM1811 Voice",
		.stream_name = "Voice Tx/Rx",
		.cpu_dai_name = "c2_eur.cp",
		.codec_dai_name = "wm8994-aif2",
		.platform_name = "snd-soc-dummy",
		.codec_name = "wm8994-codec",
		.ops = &c2_eur_wm1811_aif2_ops,
		.ignore_suspend = 1,
	},
	{
		.name = "c2_eur_WM1811 BT",
		.stream_name = "BT Tx/Rx",
		.cpu_dai_name = "c2_eur.bt",
		.codec_dai_name = "wm8994-aif3",
		.platform_name = "snd-soc-dummy",
		.codec_name = "wm8994-codec",
		.ops = &c2_eur_wm1811_aif3_ops,
		.ignore_suspend = 1,
	},
	{ /* Primary DAI i/f */
		.name = "WM8994 AIF1",
...
static int c2_eur_card_suspend_pre(struct snd_soc_card *card)
{
...
static int c2_eur_card_suspend_post(struct snd_soc_card *card)
{
	struct snd_soc_codec *codec = card->rtd->codec;
...
static int c2_eur_card_resume_pre(struct snd_soc_card *card)
{
	struct snd_soc_codec *codec = card->rtd->codec;
	struct snd_soc_dai *aif1_dai = card->rtd[0].codec_dai;
	int ret;
...
static int c2_eur_card_resume_post(struct snd_soc_card *card)
{
	struct snd_soc_codec *codec = card->rtd->codec;
	struct wm8994_priv *wm8994 = snd_soc_codec_get_drvdata(codec);
...
static struct snd_soc_card c2_eur_card = {
	.name = "c2_eur_WM1811",
	.dai_link = c2_eur_dai,

	/* If you want to use sec_fifo device,
	 * changes the num_link = 2 or ARRAY_SIZE(c2_eur_dai). */
	.num_links = ARRAY_SIZE(c2_eur_dai),

	.suspend_pre = c2_eur_card_suspend_pre,
	.suspend_post = c2_eur_card_suspend_post,
	.resume_pre = c2_eur_card_resume_pre,
	.resume_post = c2_eur_card_resume_post
};

static struct platform_device *c2_eur_snd_device;

static int __init c2_eur_audio_init(void)
{
	struct wm1811_machine_priv *wm1811;
	int ret;
	...
	snd_soc_card_set_drvdata(&c2_eur_card, wm1811);

	c2_eur_snd_device = platform_device_alloc("soc-audio", -1);
	if (!c2_eur_snd_device) {
		ret = -ENOMEM;
		goto err_device_alloc;
	}

	ret = snd_soc_register_dais(&c2_eur_snd_device->dev, c2_eur_ext_dai,
						ARRAY_SIZE(c2_eur_ext_dai));
	if (ret != 0)
		pr_err("Failed to register external DAIs: %d\n", ret);

	platform_set_drvdata(c2_eur_snd_device, &c2_eur_card);

	ret = platform_device_add(c2_eur_snd_device);
	if (ret)
		platform_device_put(c2_eur_snd_device);
...
module_init(c2_eur_audio_init);

static void __exit c2_eur_audio_exit(void)
{
	struct snd_soc_card *card = &c2_eur_card;
	struct wm1811_machine_priv *wm1811 = snd_soc_card_get_drvdata(card);
	platform_device_unregister(c2_eur_snd_device);
	kfree(wm1811);
}
module_exit(c2_eur_audio_exit);

MODULE_AUTHOR("Uk Kim <w0806.kim@samsung.com>");
MODULE_DESCRIPTION("ALSA SoC c2_eur WM1811");
MODULE_LICENSE("GPL");
