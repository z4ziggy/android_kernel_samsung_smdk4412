...
static int m9mo_set_capture_mode(struct v4l2_subdev *sd, int val)
{
	...
	case RUNNING_MODE_LOWLIGHT:
		...
		if (FRM_RATIO(state->preview) == CAM_FRMRATIO_HD) {
			err = m9mo_writeb(sd, M9MO_CATEGORY_PARM, 0x01, 0x21);
			CHECK_ERR(err);
		} else {
			err = m9mo_writeb(sd, M9MO_CATEGORY_PARM, 0x01, 0x34);
			CHECK_ERR(err);
		}
...
static int m9mo_set_CAF(struct v4l2_subdev *sd, int val)
{
	...
	if (val == state->caf_state) {
		cam_trace("same value return !!!\n");
		return 0;
	}
