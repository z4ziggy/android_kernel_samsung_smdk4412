...
static int save_gyro_caldata(struct ssp_data *data, s16 *iCalData)
{
	...
	iRet = cal_filp->f_op->write(cal_filp, (char *)&data->gyrocal,
		3 * sizeof(int), &cal_filp->f_pos);
	if (iRet != 3 * sizeof(int)) {
		pr_err("%s: [SSP] Can't write the gyrocal to file\n", __func__);
		iRet = -EIO;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((iDelayCnt >= 150) || (iRet != true)) {
		pr_err("%s: [SSP] Gyro Temp Timeout!!\n", __func__);
		goto exit;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((iDelayCnt >= 150) || (iRet != true)) {
		pr_err("%s: [SSP] Gyro Selftest Timeout!!\n", __func__);
		goto exit;
	}

	iNOST[0] = (data->uFactorydata[0] << 8) + data->uFactorydata[1];
	iNOST[1] = (data->uFactorydata[2] << 8) + data->uFactorydata[3];
	iNOST[2] = (data->uFactorydata[4] << 8) + data->uFactorydata[5];

	iST[0] = (data->uFactorydata[6] << 8) + data->uFactorydata[7];
	iST[1] = (data->uFactorydata[8] << 8) + data->uFactorydata[9];
	iST[2] = (data->uFactorydata[10] << 8) + data->uFactorydata[11];

	iCalData[0] = (data->uFactorydata[12] << 8) + data->uFactorydata[13];
	iCalData[1] = (data->uFactorydata[14] << 8) + data->uFactorydata[15];
	iCalData[2] = (data->uFactorydata[16] << 8) + data->uFactorydata[17];
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((iDelayCnt >= 150) || (iRet != true)) {
		pr_err("%s: [SSP] Gyro Selftest DPS Timeout!!\n", __func__);
		goto exit;
	}

	if (data->uFactorydata[0] == false) {
		pr_err("%s: [SSP] Gyro Selftest DPS Error!!\n", __func__);
		goto exit;
	}
