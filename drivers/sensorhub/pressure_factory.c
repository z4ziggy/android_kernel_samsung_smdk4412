	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	input_report_abs(data->pressure_input_dev, ABS_HAT3X,
...
int pressure_open_calibration(struct ssp_data *data)
{
	...
	if (IS_ERR(cal_filp)) {
		...
		if (iErr != -ENOENT)
			pr_err("%s: Can't open calibration file(iErr=%d)\n",
				__func__, iErr);
		set_fs(old_fs);
		return iErr;
	}
	...
	if (iErr < 0) {
		pr_err("%s: Can't read the cal data from file (iErr=%d)\n",
			__func__, iErr);
		return iErr;
	}
	...
	if (iErr < 0) {
		pr_err("[SSP]: %s kstrtoint failed. iErr = %d", __func__, iErr);
		return iErr;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (iErr < 0) {
		pr_err("%s, kstrtoint failed.(iErr = %d)", __func__, iErr);
		return iErr;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((iDelayCnt >= 150) || (iRet != true)) {
		pr_err("%s: [SSP] Pressure Selftest Timeout!!\n",
			__func__);
		goto exit;
	}
