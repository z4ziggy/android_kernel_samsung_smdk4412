...
static int cbp72_download_bin(struct dpram_link_device *dpld,
			struct sk_buff *skb)
{
...
static int cmc221_download_bin(struct dpram_link_device *dpld,
			struct sk_buff *skb)
{
	if (dpld->dp_type == CP_IDPRAM)
