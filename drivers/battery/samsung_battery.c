...
static int battery_get_curr_avg(struct battery_info *info)
{
	...
	pwroff_min_vol = info->pdata->voltage_min - PWROFF_MARGIN;

	/* if soc 0% && under poweroff min voltage && USB charging, power off */
	if ((info->battery_soc <= PWROFF_SOC) &&
		(info->battery_vcell < pwroff_min_vol) &&
		(info->cable_type == POWER_SUPPLY_TYPE_USB)) {
		pr_info("%s: vol %dmV(< %dmV), but usb chging\n", __func__,
			(info->battery_vcell / 1000), (pwroff_min_vol / 1000));
		curr_avg = -1;
	} else {
...
static __devinit int samsung_battery_probe(struct platform_device *pdev)
{
	...
	char *vf_src_name[] = { "adc", "charger irq", "unknown"
	};
